...
## 진행 중 — App Store 심사 대기 + 출시 후속 (2026-07-26 제출)

**코드·TestFlight·메타데이터·심사 제출 완료.** 남은 건 심사 결과 대기와 출시 후속(광고 재확인·코드 개선). 배포 파이프라인 정보는 아래 참고용으로 유지.
...
- ✅ **로그인 3종 실제 동작 확인됨**(Google/Kakao/네이버, 사용자 실기기 확인 — #7/#8 미검증 리스크 해소).
- **남은 QA**: 실광고 노출은 **출시 후 재확인**(미출시 앱은 AdMob 실광고 제한 추정, DEBUG 테스트광고는 정상).

### 심사 제출 완료 (2026-07-26)
App Store Connect 메타데이터 전부 입력 후 **심사 제출**. 상태: 심사 대기.
- 한/영 로컬라이제이션: 설명·부제·키워드·프로모션·"새 소식".
- 스크린샷 6.5"(1284×2778) — 6.3"(17 Pro) 원본을 리사이즈. 6.9" 등은 ASC가 자동 축소 재사용.
- 개인정보 처리방침 URL 한/영(`https://clipnote.co.kr/privacy`).
- 연령등급 **4+**(전 항목 None, 웹접근 No), Content Rights(제3자 콘텐츠 있음), 가격 **무료**.
- App Privacy 게시: 이메일·userID·기타 사용자콘텐츠=앱기능/신원연결/미추적, 기기ID=제3자광고/미연결/미추적.
- 심사메모: 로그인 없이 게스트로 전체 기능 사용 가능(데모 계정 불필요) 명시.

### 출시 후속 (심사 후)
- **심사 결과 대기**(24~48h) → 승인 시 릴리스(자동/수동 여부 확인 필요). 반려 시 사유 대응 — **Sign in with Apple(4.8)** 지적 가능성(게스트 사용 가능 + 심사메모로 방어).
- **광고 정상화**(출시 후): 실기기 실광고 재확인, AdMob `app-ads.txt`(`clipnote.co.kr/app-ads.txt`) 설정, AdMob 앱 "Ready" 확인.
- **코드 후속**: `AdConfig.enabled`가 App ID만 검사하고 배너 unit ID는 미검사 → unit ID 비면 배너가 조용히 안 뜸(보강 여지). Sign in with Apple 선제 추가 여지. Privacy Manifest AdMob 재검토(`NSPrivacyTracking` 현재 false).
- 앱 아이콘: 현재 512→1024 업스케일본, 필요 시 1024 원본 교체.
