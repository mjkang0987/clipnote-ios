...
- **1.1.0 심사 제출 완료(2026-08-04)** — 빌드 **22** 로 제출. 상태: **심사 대기**.
  - 빌드 22 를 고른 이유: 21 에는 광고 고지가 빠진 옛 개인정보처리방침이 들어 있어
    ASC 의 App Privacy 신고(기기ID = 제3자 광고)와 어긋난다. 실기기 구동도 22 로 확인했다.
  - 스토어 목록 정보(부제·설명·키워드·프로모션·릴리스 노트)는 저장소가 원본이다
    (`fastlane/metadata/`). 스크린샷·카테고리·연령등급·개인정보 URL 은 1.0 제출 때 넣은
    값을 그대로 쓴다 — **앱 단위 정보라 버전이 바뀌어도 유지된다.**
- **미완/이월(사람만 가능)**:
  - 심사 결과 대기 → 승인 시 출시. 반려 시 사유 대응.
  - **실광고 노출 확인** — TestFlight 는 DEBUG 가 아니지만, 실제 노출은 스토어 출시 후 재확인.
  - Privacy Manifest는 AdMob 포함 상태 재검토 여지(`NSPrivacyTracking`/추적 도메인 — 현재 false).
    개인정보처리방침에는 광고 수집·위탁을 고지했으므로(2026-08-04) 매니페스트만 남았다.
  - 스토어 스크린샷은 1.0 때 것이라 화면이 바뀐 부분이 있다(홈 헤더·온보딩 투어·날짜 그룹).
    갱신은 선택 사항.
